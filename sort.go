// Package externalsort provides a facility for performing disk-based external
// merge sorts.
//
...
// for sorting.
type Chunk func(seeker io.ReadSeeker) ([]byte, error)
...
// the amount of RAM used to approximately memLimit (in bytes).
...
func (s *sorted) Close() error {
	...
	files := make(map[int]*os.File, s.numFiles)
...
func (s *sorted) finalSort(files map[int]*os.File) error {
